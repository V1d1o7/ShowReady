...
const getLayoutedElements = (nodes, edges, direction = 'LR') => {
  ...
  nodes.forEach((node) => {
    // Make sure width and height are available, otherwise use a default
...
const WireDiagramView = ({ showName }) => {
    ...
    const onLayout = useCallback(() => {
        ...
        // Save the new positions to the backend
    ...
    const fetchData = useCallback(async () => {
        ...
        setError(null);
        try {
            // Fetch all equipment for the show
            ...
            for (const rack of racksData) {
                ...
                // Add rack info to each piece of equipment
                const equipmentInRack = detailedRack.equipment.map(eq => ({
                    ...eq,
                    rack_name: detailedRack.rack_name,
                }));
                allEquipment = [...allEquipment, ...equipmentInRack];
            }

            // Fetch all connections for the show
            const connectionsData = await api.getConnectionsForShow(showName);
            const connections = connectionsData.connections || [];

            // Create React Flow nodes from equipment
            const initialNodes = allEquipment.map((item, index) => ({
                id: item.id.toString(),
                type: 'device',
                position: {
                    x: item.x_pos || (index % 4) * 450, // Default positioning
                    y: item.y_pos || Math.floor(index / 4) * 350,
                },
                data: {
                    ...item,
    ...
    const onConnect = useCallback(async (params) => {
        ...
        if (sourceHandle.includes('in') || targetHandle.includes('out')) {
            return;
        }
    ...
    if (isLoading) {
        return <div className="flex items-center justify-center h-full"><div className="text-xl text-gray-400">Loading Wire Diagram...</div></div>;
    }
    ...
    return (
        <div className="h-[calc(100vh-220px)] w-full rounded-xl bg-gray-800/50 relative" data-testid="wire-diagram-view">
            <div className="absolute top-4 right-4 z-10 flex gap-2">
                <button
                    onClick={onLayout}
                    className="flex items-center gap-2 px-4 py-2 text-sm bg-gray-600 hover:bg-gray-500 rounded-lg font-bold text-white shadow-lg"
                >
                    Auto-Arrange
                </button>
                <button
                    onClick={() => setIsPdfModalOpen(true)}
                    className="flex items-center gap-2 px-4 py-2 text-sm bg-amber-500 hover:bg-amber-400 rounded-lg font-bold text-black shadow-lg"
                >
                    <Download size={16} />
                    Generate PDF
                </button>
